...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    } else if (hideDefaultCursor && !container) {
      document.body.style.cursor = 'none';
    }

    // Start hidden if scoped to container
    if (container) {
    ...
    // Container enter/leave for scoped mode
    ...
    const mouseDownHandler = () => {
      if (!dotRef.current || (container && !isInsideContainerRef.current)) return;
    ...
    const enterHandler = e => {
      if (container && !isInsideContainerRef.current) return;
    ...
    return () => {
      ...
      if (container) {
        container.removeEventListener('mouseenter', containerEnter);
  ...
  }, [targetSelector, containerSelector, spinDuration, moveCursor, constants, hideDefaultCursor, isMobile, hoverDuration, parallaxOn]);
  ...
  const borderStyle = `3px solid ${cornerColor}`;
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderTop: borderStyle, borderLeft: borderStyle,
      ...
      <div
        ...
        style={{
          position: 'absolute', top: '50%', left: '50%',
          width: '12px', height: '12px',
          borderBottom: borderStyle, borderLeft: borderStyle,
